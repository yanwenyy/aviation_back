  ...
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="图片">
        <template slot-scope="scope">
          <img  @click="preImg(scope.row.imgUrl&&scope.row.imgUrl.indexOf('http')!=-1?scope.row.imgUrl:imgUrlfront+scope.row.imgUrl)" class="table-list-img" v-if="scope.row.imgUrl" :src="scope.row.imgUrl&&scope.row.imgUrl.indexOf('http')!=-1?scope.row.imgUrl:scope.row.imgUrl?imgUrlfront+scope.row.imgUrl:''" alt="">
        </template>
      </el-table-column>
      <el-table-column
        prop="sort"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="" type="text" size="small" @click="ifOnLine(scope.row.id,scope.row.showFlag)">{{scope.row.showFlag==0?'下线':'上线'}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ifOnLine (id,status) {
        this.$http({
          url: this.$http.adornUrl(`/biz/banner/info/${status==0?'1':'0'}/${id}`),
          method: 'GET'
        }).then(({data}) => {
          if (data && data.code === 10000) {
            this.$message({
