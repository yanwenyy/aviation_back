...
<script>
  import AddOrUpdate from './memorabilia-add-or-update'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      ifOnLine (id,status) {
        this.$http({
          url: this.$http.adornUrl(`/biz/bigevent/info/${status==0?'1':'0'}/${id}`),
          method: 'GET'
        }).then(({data}) => {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/bigevent/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确认删除该条数据吗?删除后数据不可恢复`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/biz/bigevent/delete/'+id),
            method: 'GET',
          }).then(({data}) => {
            if (data && data.code === 10000) {
              this.$message({
