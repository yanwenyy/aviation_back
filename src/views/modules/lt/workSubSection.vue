  ...
  <div class="mod-user">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item label="权限:">
        <el-select clearable  v-model="dataForm.purviewType" placeholder="请选择">
          <el-option
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          {{ scope.row.purviewType==0?'全部会员 ':'指定会员'}}
        </template>
      </el-table-column>
      <el-table-column
        prop="purviewNum"
...
<script>
  import AddOrUpdate from './workSubSection-add-or-update'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      ifOnLine (id,status) {
        this.$http({
          url: this.$http.adornUrl(`/biz/jobchildmodel/info/${id}/${status==0?'1':'0'}`),
          method: 'GET'
        }).then(({data}) => {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/jobchildmodel/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确认删除该条数据吗?删除后数据不可恢复`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/biz/jobchildmodel/delete'),
            method: 'POST',
